        ...
        <section class="card step-card step-3 <?=empty($_GET["bloque_id"])?"locked":""?>">
            <h2><b>3</b> Datos y medio de pago</h2><?php if ($error): ?><p><?= htmlspecialchars($error) ?></p><?php elseif ($confirmado): ?><h3>Reserva #<?= $confirmado ?> creada</h3>
                <p>Pago pendiente de completar.</p><?php if ((int)($_POST['medio_pago_id'] ?? 0) === 1): ?><div class="transfer-info"><strong>Transferencia bancaria</strong>
                        ...
                        <?php $transferEmail=(string)env('PARTICULARES_TRANSFER_EMAIL', 'pagos@maiteam.cl'); $transferWhatsapp=preg_replace('/[^0-9]/', '', (string)env('PARTICULARES_TRANSFER_WHATSAPP', '')); $whatsappMessage='Hola, envié el comprobante de la reserva #'.$confirmado.' de clases particulares.'; ?>
                        <p>Envía el comprobante a <a href="mailto:<?= htmlspecialchars($transferEmail) ?>"><?= htmlspecialchars($transferEmail) ?></a>.</p>
                        <?php if ($transferWhatsapp !== ''): ?><a class="button whatsapp-button" target="_blank" rel="noopener" href="https://wa.me/<?= htmlspecialchars($transferWhatsapp) ?>?text=<?= rawurlencode($whatsappMessage) ?>">Notificar pago por WhatsApp</a><?php endif; ?>
